...
class StockStatus extends \Opencart\System\Engine\Model {
	...
	public function addStockStatus(array $data): ?int {
		...
		foreach ($data['stock_status'] as $language_id => $stock_status) {
			if (!$stock_status_id) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "stock_status` SET `language_id` = '" . (int)$language_id . "', `name` = '" . $this->db->escape($stock_status['name']) . "', `item_availability` = '" . $this->db->escape($stock_status['item_availability']) . "'");
