...
class Product extends \Opencart\System\Engine\Controller {
	...
	public function index(array $product_data): string {
		...
		if(!empty($product_data['width']) && is_numeric($product_data['width']) && floatval($product_data['width']) > 0){
			$data['structured_data']['width'] = [
				"@type" => "QuantitativeValue",
				"value" => floatval($product_data['width']),
				"unitText" => $product_data['length_unit']
			];
		}

		if(!empty($product_data['height']) && is_numeric($product_data['height']) && floatval($product_data['height']) > 0){
			$data['structured_data']['height'] = [
				"@type" => "QuantitativeValue",
				"value" => floatval($product_data['height']),
				"unitText" => $product_data['length_unit']
			];
		}

		if(!empty($product_data['depth']) && is_numeric($product_data['depth']) && floatval($product_data['depth']) > 0){
			$data['structured_data']['depth'] = [
				"@type" => "QuantitativeValue",
				"value" => floatval($product_data['depth']),
				"unitText" => $product_data['length_unit']
			];
		}

		if(!empty($product_data['weight']) && is_numeric($product_data['weight']) && floatval($product_data['weight']) > 0){
			$data['structured_data']['weight'] = [
				"@type" => "QuantitativeValue",
				"value" => floatval($product_data['weight']),
		...
		if(!empty($product_data['rating']['review_count'])){
			$data['structured_data']['aggregateRating'] = [
				...
				"ratingValue" => floatval($product_data['rating']['average']),
				"reviewCount" => intval($product_data['rating']['review_count']),
			];
		}
