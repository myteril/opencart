...
class Install extends \Opencart\System\Engine\Model
{
	...
	public function setupDatabaseData(\Opencart\System\Library\DB $db, string $data_sql_file, string $db_prefix, string $admin_username, string $admin_password, string $admin_email): void
	{
		...
		if ($lines) {
			...
			$db->query("INSERT INTO `" . $db_prefix . "user` SET `user_id` = '1', `user_group_id` = '1', `username` = '" . $db->escape($admin_username) . "', `password` = '" . $db->escape(password_hash(html_entity_decode($admin_password, ENT_QUOTES, 'UTF-8'), PASSWORD_DEFAULT)) . "', `firstname` = 'John', `lastname` = 'Doe', `email` = '" . $db->escape($admin_email) . "', `status` = '1', `date_added` = NOW()");

			$db->query("UPDATE `" . $db_prefix . "setting` SET `code` = 'config', `key` = 'config_email', `value` = '" . $db->escape($admin_email) . "' WHERE `key` = 'config_email'");
